classdef DatacubeReductionAutomated < DataReduction
    properties (Constant)
        Name = 'Datacube';
        Description = '';
    ...
    methods
        function obj = DatacubeReductionAutomated(imageGeneration, output)
...
%             images = dataRepresentation.parser.getImages(centroids, peakWidths);
        ...
        function dataInMemory = generateDataRepresentationList(this, dataRepresentation, peakList, data, rois)
            dataRepresentationList = DataRepresentationList();
            ...
            for i = 1:size(pixels, 1)
                ...
                if(isempty(imzMLList))
                    ...
                    if(this.processEntireDataset)
                        ...
                        defaults{end+1} = [filePath filesep 'Segmented' filesep fileName '_tissue.imzML'];
                        pixelLists{end+1} = pixels;
                    end
                    for roiIndex = 1:numel(rois)
                        prompt{end+1} = rois{roiIndex}.getName();
                        defaults{end+1} = [filePath filesep 'Segmentation Matching' filesep fileName 'K' num2str(numel(rois)) filesep fileName '_' rois{roiIndex}.getName() '.imzML'];
                        pixelLists{end+1} = rois{roiIndex}.getPixelList();
                    end
                    
                    filenameList = defaults;
                    ...
                    for fileIndex = 1:numel(filenameList)
                        ...
                        filenameList{fileIndex} = char(filenameList{fileIndex});
